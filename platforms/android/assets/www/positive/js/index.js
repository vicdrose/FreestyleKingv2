...
/*    commenting out name
    if ( name !== "" && string.indexOf("pronoun1") === 0 ) {
      ...
      /* * cycle through all words,
           * replace instances of each array index with another array index
        */
        return string;
      }
...
randomise
scripts.js:20 amita
scripts.js:20 lobster
scripts.js:20 bungieinline
scripts.js:20 indieflower
scripts.js:20 monospace
*/

function changeFont() {
    console.log("changeFont called");
    defaultFont = this.value;
    return fontCall(defaultFont);
}

function fontCall(font) {
  console.log("fontCall called");
  if ( font === "monospace") {
    return affirmationText.className = "monospace";
  } else if ( font === "indieflower") {
    return affirmationText.className = "indieflower";
  } else if ( font === "bungieinline") {
    return affirmationText.className = "bungieinline";
  } else if ( font === "lobster") {
    return affirmationText.className = "lobster";
  } else if ( font === "amita") {
    return affirmationText.className = "amita";
  } else if ( font === "randomise" ) {
    randomiseFont(numNum);
  }
}

function randomiseFont(ranNum) {
  console.log("randomiseFont called");
  ranNum = Math.floor(Math.random() * 5);
  console.log(ranNum + " random");
  if ( ranNum === 0 ) {
    return fontCall("monospace");
  } else if ( ranNum === 1 ) {
    return fontCall("indieflower");
  } else if ( ranNum === 2 ) {
    return fontCall("bungieinline");
  } else if ( ranNum === 3 ) {
    return fontCall("amita");
  } else if ( ranNum === 4) {
    return fontCall("lobster");
  }
}

function generateRandom() {
  console.log("randomise called");
 numNum = Math.floor(Math.random() * 5);
 console.log(numNum);
 return randomiseFont(numNum);
}
/* string modification */
...
function capitaliseAfterFullStop(string) {
  if ( string.indexOf(".") !== -1 ) {
      var periodIndex = string.indexOf(".");
      if ( string.charAt(periodIndex+1) !== "." ) {
      string = string.slice(0, periodIndex + 2 ) + string.charAt( periodIndex + 2 ).toUpperCase() + string.slice( periodIndex + 3 );
    }
  }
  return string;
}

function capitaliseLetterI(string) {
  return string.replace(" i ", " I ");
}

/* event listeners */
changeAffButton.addEventListener("click", changeAffirmation);
document.addEventListener('keypress', function (enter) { var key = enter.which || enter.keycode; if ( key == 13 ) { return changeAffirmation();} })
/* addNameButton.addEventListener("click", insertName);*/
/* console.log(arrPositiveYou1[70]); */
/*
<h5>If choosing she, he or they you may optionally input your name.</h5>
<input type="text" id="name-input"></input><button id="add-name-button">Add name.</button>
*/